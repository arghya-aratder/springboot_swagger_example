    ...
    stages {
        ...
        stage('Build') {
            steps {
                echo 'Clean Build'
                    ...
                    sh "mvn sonar:sonar clean compile -Dtest=\\!TestRunner* -DfailIfNoTests=false -Dsonar.projectKey=employee_jdbc -Dsonar.host.url=http://35.200.239.255/ -Dsonar.login=aac7cc7809ddc82ce0070e3f74726c71216936b6"
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'curl --upload-file target/spring-boot-web-0.0.1-SNAPSHOT.war "http://tomcat:password@34.93.179.229:8081/manager/text/deploy?path=/swaggerapi&update=true"'
        ...
        /*stage('Artifact Promotion') {
            steps {
                ...
                debug: false,
                    ...
                    stagingRepository: 'http://35.200.161.61:8081/repository/maven-snapshots',
                    stagingUser: 'admin',
                    stagingPW: 'admin123',
                    skipDeletion: true,
